...
function CartPage() {
  ...
  useEffect(() => {
    load().catch((error) => {
      ...
      setMessage(error.response?.data?.error || 'Failed to load cart data.');
    });
  ...
  const placeOrder = async () => {
    try {
      ...
      setMessage(error.response?.data?.error || 'Order placement failed');
    }
  };
      ...
      {cart.items.length === 0 ? (
        ...
        <div className="mt-6 space-y-4">
          ...
          {cart.items.map((item) => (
            <div
              key={item.id}
              className="bg-white rounded-xl border border-slate-200 p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3"
            >
              <div>
            ...
            >
              ...
              {prescriptions.map((p) => (
                <option key={p.id} value={p.id}>
                  {p.fileName} ({p.validated ? 'Validated' : 'Pending'})
                </option>
              ))}
            </select>
          </div>

          <button onClick={placeOrder} className="px-5 py-3 rounded-xl bg-brand-700 text-white font-semibold">
            Place Order
          </button>
