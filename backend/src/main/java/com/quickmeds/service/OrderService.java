...
public class OrderService {
    ...
    public OrderDtos.OrderResponse place(String username, OrderDtos.PlaceOrderRequest request) {
        ...
        for (CartItem item : cart.getItems()) {
            ...
            BigDecimal lineTotal = medicine.getPrice().multiply(BigDecimal.valueOf(item.getQuantity()));
            subtotal = subtotal.add(lineTotal);

            Optional<Offer> lineOffer = selectedOffer;
            if (lineOffer.isPresent() && lineOffer.get().getCategory() != null
                    && !lineOffer.get().getCategory().getId().equals(medicine.getCategory().getId())) {
        ...
        int requestedPoints = request.getPointsToRedeem() != null ? request.getPointsToRedeem().intValue() : 0;
        ...
        return toResponse(saved, subtotal, offerDiscount.add(loyaltyDiscount), pointsToRedeem, pointsEarned, selectedOffer.orElse(null));
    }
    ...
    private OrderDtos.OrderResponse toResponse(Order o, BigDecimal originalAmount, BigDecimal discountAmount,
                                               Integer loyaltyPointsUsed, Integer loyaltyPointsEarned, Offer offer) {
        return OrderDtos.OrderResponse.builder()
                ...
                .deliveryAddress(o.getAddress() != null
                    ? String.format("%s, %s, %s, %s", o.getAddress().getStreet(), o.getAddress().getCity(), o.getAddress().getState(), o.getAddress().getPincode())
                    : null)
